...
- Head over to your ```app.js``` file or wherever you're initiating your new app and right above your ```.config``` method, add a ```.constant``` method with the first argument being "fb" (which is the name of the constant) and the second argument being an object with a key of "url" whose value is .
